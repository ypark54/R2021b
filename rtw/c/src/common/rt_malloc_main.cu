...
# error "Classic call interface is not supported by rt_malloc_main.cu."
#endif

#if ONESTEPFCN==0
#error Separate output and update functions are not supported by rt_malloc_main.cu. \
You must update rt_malloc_main.cu to suit your application needs, or select \
the 'Single output/update function' option.
#endif

#if TERMFCN==0
#error The terminate function is required by rt_malloc_main.cu. \
Select model configuration parameter 'Terminate function required' \
or 'Generate an example main program' or modify rt_malloc_main.cu to \
meet your application requirements.
#endif

#if ALLOCATIONFCN==0 
# error An allocation function is required by rt_malloc_main.cu. \
You must update rt_malloc_main.cu to allocate model instance data, or select \
...
#error The static version of rt_malloc_main.cu does not support model step function prototype control.
#endif

#if ROOT_IO_FORMAT != 2
# error rt_malloc_main.cu requires root-level I/O to be passed as part of model \
data structure.                                                                \
You must update rt_malloc_main.cu  to suit your application needs, or set        \
