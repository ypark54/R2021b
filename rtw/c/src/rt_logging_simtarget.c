 ...
 */
static const char_T *rt_FixupLogVar(LogVar *var, boolean_T isVarDims, int verbose)
{
...
void rt_StopDataLoggingImpl(const char_T *file, RTWLogInfo *li, boolean_T isRaccel)
{
     ...
     **************************************************/
    while (var != NULL) {
        if ( (msg = rt_FixupLogVar(var, (var->valDims != NULL), verbose)) != NULL ) {
    ...
    while (svar != NULL) {
        ...
        if (svar->logTime) {
            var = svar->time;
            if ( (msg = rt_FixupLogVar(var, false, verbose)) != NULL ) {
                (void)fprintf(stderr, "*** Error writing %s due to: %s\n",
                              file, msg);
                errFlag = 1;
                break;
        ...
        i = 0;
        while (var) {
            if ( (msg = rt_FixupLogVar(var, isVarDims[i], verbose)) != NULL ) {
                (void)fprintf(stderr, "*** Error writing %s due to: %s\n",
                              file, msg);
                errFlag = 1;
                break;
