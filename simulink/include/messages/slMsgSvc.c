...
#   define __slmsg_POOLED_FREE(pool, ptr, poolId)     __slmsg_MemPool_Free (pool, ptr, poolId)
#else
...
/* -----------------------------------------
* Data-pointer pool handling
*/
int _slMsgIsDataPointerPool(__slmsg_MemPool *pool)
{
    return (pool->fPointerDataType != -1);
}

void _slMsgDataPointerConstruct(__slmsg_MemPool *pool, void *msgData)
{
    __slmsg_assert(_slMsgIsDataPointerPool(pool));
    __slmsg_assert(pool->fPointerConstruct != NULL);
    pool->fPointerConstruct(pool, msgData);
}

void _slMsgDataPointerDestruct(__slmsg_MemPool *pool, void *msgData)
{
    __slmsg_assert(_slMsgIsDataPointerPool(pool));
    __slmsg_assert(pool->fPointerDestruct != NULL);
    pool->fPointerDestruct(pool, msgData);
}

void _slMsgDataPointerDeepCopy(__slmsg_MemPool *pool, void *dstData, const void* srcData)
{
    __slmsg_assert(_slMsgIsDataPointerPool(pool));
    __slmsg_assert(pool->fPointerCopy != NULL);
    pool->fPointerCopy(pool, dstData, srcData);
}
...
void  __slmsg_MemPool_Free(__slmsg_MemPoolMgr *mgr, void *ptr, slMsgMemPoolId poolId);
                         ...
                         void *memBlock)
{
    ...
    /* Initialize data-pointer pool related properties */
    pool->fPointerDataType = -1;
    pool->fDataTypeTable = NULL;
    pool->fNumElements = 0;
    pool->fPointerConstruct = NULL;
    pool->fPointerDestruct = NULL;
    pool->fPointerCopy = NULL;
...
void __slmsg_private_MemPool_Free(__slmsg_MemPool *pool, void **ptrIn)
{
    ...
    if (memFromPool) {
        ...
        if (_slMsgIsDataPointerPool(pool)) {
            _slMsgDataPointerDestruct(pool, ptr);
        }
        else if (_slMsgNeedDataHandling(pool)) {
...
void __slmsg_MemPool_Free(__slmsg_MemPoolMgr *mgr, void *ptr, slMsgMemPoolId poolId)
{
    __slmsg_private_MemPool_Free(&(mgr->fPools[poolId]), &ptr);
...
boolean_T _slMsgSvcIsMsgInQueue(slMsgManager *msgMgr, slMsgQueueId queueId, slMessage *msg);
...
void _slMsgDestroy(slMsgManager *msgMgr, slMessage *msg)
{
    ...
    __slmsg_POOLED_FREE(&msgMgr->fPoolMgr, msg->fData, msg->fDataPoolId);
    __slmsg_POOLED_FREE(&msgMgr->fPoolMgr, msg, msg->fMsgPoolId);
}
                                    ...
                                    slMsgId msgId)
{
    ...
    if (_slMsgIsDataPointerPool(msgWriterPayloadPool)) {
        _slMsgDataPointerConstruct(msgWriterPayloadPool, msg->fData);
    }
    else if (_slMsgNeedDataHandling(msgWriterPayloadPool)) {
    ...
    if (data == NULL) {
        ...
        if (_slMsgIsDataPointerPool(msgWriterPayloadPool)) {
            _slMsgDataPointerDeepCopy(msgWriterPayloadPool, msg->fData, data);
        }
        else if (_slMsgNeedDataHandling(msgWriterPayloadPool)) {
    ...
    msg->fMsgPoolId  = queue->writerMessageMemPoolId;
    msg->fDataPoolId = queue->writerPayloadMemPoolId;
...
slMessage *_slMsgSvcSRSWSendMsg(slMsgManager *msgMgr, slMessage *msg, slMsgQueueId queueId)
{
    ...
    if (SLMSG_CIRCULAR_INDEX(
        ...
        if (_slMsgIsDataPointerPool(msgWriterPool)) {
            _slMsgDataPointerDeepCopy(msgWriterPool, dst, msg->fData);
        }
        else if (_slMsgNeedDataHandling(msgWriterPool)) {
...
boolean_T _slMsgSvcIsMsgInQueue(slMsgManager* msgMgr,
                                slMsgQueueId queueId,
                                slMessage* inMsg)
{
    slMessage* msg = NULL;
    ...
    if (q->fLength > 0) {
        ...
        while ((msgCounter != q->fLength) && (msg != NULL)) {
            if (msg == inMsg)
                return true;
            msg = msg->fNext;
            msgCounter++;
...
{
    __slmsg_MemPool* pool = NULL;

    pool = &(mgr->fPoolMgr.fPools[msg->fDataPoolId]);

    if (_slMsgIsDataPointerPool(pool)) {
        _slMsgDataPointerDeepCopy(pool, buf, msg->fData);
    }
    else if (_slMsgNeedDataHandling(pool)) {
        __slmsg_assert(_slMsgCallDeepCopy(pool, buf, msg->fData));
    }
    else {
...
boolean_T _des_slMsgSvcIsMsgInQueue(slMsgManager* mgr, slMsgQueueId qId, slMessage* msg)
{
    return _slMsgSvcIsMsgInQueue(mgr, qId, msg);
}
...
uint_T _des_slMsgSvcQueueIsOverflow(slMsgManager* msgMgr, slMsgQueueId queueId)
{
    return slMsgSvcQueueIsOverflow(msgMgr, queueId);
}

void _des_slMsgSvcQueueSetIsOverflow(slMsgManager* msgMgr, slMsgQueueId queueId, uint_T val)
{
    slMsgSvcSetQueueIsOverflow(msgMgr, queueId, val);
}
