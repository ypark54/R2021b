...
mZcDisabled;};typedef struct NeSolverParametersTag NeSolverParameters;typedef
enum NeLocalSolverChoiceTag{NE_INVALID_ADVANCER_CHOICE= -1,
...
;NePartitionMethod mPartitionMethod;};typedef enum{DAEMON_CHOICE_INVALID= -1,
DAEMON_CHOICE_NONE,DAEMON_CHOICE_LOG_NUM_ITERATIONS,DAEMON_CHOICE_SSC2HDL,
NUM_DAEMON_CHOICES}DaemonChoice;typedef enum NeEquationDomainTag{
...
boolean_T mIsTop;}NeParameterData;typedef struct NeConstraintStatTag
NeConstraintStat;struct NeConstraintStatTag{void(*mVariable)(const
NeConstraintStat*,const NeVariableData*);struct NeConstraintStatData*mData;};
typedef struct NeCompStatsTag NeCompStats;struct NeCompStatsTag{const
NeConstraintStat*(*mConstraint)(const NeCompStats*);struct NeCompStatsData*
mData;};
#ifdef __cplusplus
}
#endif /* __cplusplus */
...
};NeRtpValues*make_rtw_rtp_values(NeParameterBundle*pb);typedef struct
NeStiffnessTag NeStiffness;typedef struct SscIniterTag SscIniter;typedef enum
IniterTypeTag{INITER_TYPE_INVALID= -1,INITER_TYPE_AC,INITER_TYPE_AC_DAE,
INITER_TYPE_DAE,INITER_TYPE_COUNT}IniterType;typedef struct SscComputeStateTag
SscComputeState;PmfMessageId ne_dae_create(NeDae**dae,NeDynamicSystem*
dynamicsystemPtr,SscIniter*initer,boolean_T isIniter,NeRtpValues*rtpValues,
NeSolverParameters solverParameters,NeModelParameters modelParameters,const
McLinearAlgebraFactory*linearAlgebraPtr,NeProfiler*profiler,const NeCompStats*
compStats,CStateManager*csm,PmAllocator*allocatorPtr,const DaemonChoice dc);
#ifdef __cplusplus
}
#endif /* __cplusplus */
