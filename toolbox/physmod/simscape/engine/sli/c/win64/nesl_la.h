...
size_t rtw_linalg_memusage(const McLinearAlgebra* ne_la)
{
    ...
    return
        ...
        numrow * numcol * sizeof(real_T) + numrow * sizeof(real_T) +
        ((numrow < numcol) ? numrow : numcol) * sizeof(int32_T);
}
...
create_rtw_linear_algebra(const McLinearAlgebraFactory* factory,
                          ...
                          const PmSparsityPattern*      pattern)
{
    return create_rtw_linear_algebra_complete(
        factory, linAlg, pattern, pattern->mNumCol);
}
                                    ...
                                    size_t                        nPerm)
{
    ...
    if (N <= 20.0 || (N <= 100.0 && nnz / N > 0.1125 * N - 1.25) ||
        ...
        result = full_la->mCreateLinearAlgebraComplete(
            full_la, linAlg, pattern, nPerm);
    } else {
        result = sparse_la->mCreateLinearAlgebraComplete(
            sparse_la, linAlg, pattern, nPerm);
    }
...
create_auto_linear_algebra(const McLinearAlgebraFactory* factory,
                           ...
                           const PmSparsityPattern*      pattern)
{
    return create_auto_linear_algebra_complete(
        factory, linAlg, pattern, pattern->mNumCol);
}
