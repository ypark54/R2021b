                                      ...
                                      boolean_T                   firstOutput)
{
    return solver->mSolve(
        solver, inputs, states, outputs, time, NULL, mgr, firstOutput);
}
...
PMF_DEPLOY_STATIC NeslRtpManager*
                  swl_rtp_manager_create(const Simulator* simulator)
{
    return rtp_manager_create(simulator);
}

PMF_DEPLOY_STATIC SscRTWLogFcnManager*
                  swl_rtw_logging_manager_create(const Simulator*      simulator,
                                                 unsigned int          inputPortWidth,
                                                 unsigned int          bufferWidth,
                                                 NeuDiagnosticManager* diagMgr)
{
    return ssc_swl_create_rtw_log_fcn_manager(
        simulator, inputPortWidth, bufferWidth, diagMgr);
}
